...
Colors = ['green', 'blue', 'orange', 'red', 'pink', 'purple', 'magenta', 'yellow', 'hotpink', 'navy', ]
...
class Animation:
    def __init__(self, my_map, starts, goals, paths):
        ...
        self.patches.append(Rectangle((x_min, y_min), x_max - x_min, y_max - y_min, facecolor='azure', edgecolor='gray'))
        for i in range(len(self.my_map)):
            for j in range(len(self.my_map[0])):
                if self.my_map[i][j]:
                    self.patches.append(Rectangle((i - 0.5, j - 0.5), 1, 1, facecolor='gray', edgecolor='black'))
    ...
    def animate_func(self, t):
        for k in range(len(self.paths)):
            ...
            self.agent_names[k].set_position((pos[0], pos[1] + 0))
